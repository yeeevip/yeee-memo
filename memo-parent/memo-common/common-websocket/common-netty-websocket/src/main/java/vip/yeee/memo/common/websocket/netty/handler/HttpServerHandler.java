...
import vip.yeee.memo.common.websocket.netty.support.WsPathMatcher;
...
class HttpServerHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    public HttpServerHandler(WsEndpointDispatcher wsEndpointDispatcher, ServerEndpointConfig config, EventExecutorGroup eventExecutorGroup, boolean isCors) {
    ...
    private void handleHttpRequest(ChannelHandlerContext ctx, FullHttpRequest req) {
        ...
        if (StringUtils.hasText(wsEndpointDispatcher.getHost()) && !wsEndpointDispatcher.getHost().equals("0.0.0.0") && !wsEndpointDispatcher.getHost().equals(host.split(":")[0])) {
            if (forbiddenByteBuf != null) {
                res = new DefaultFullHttpResponse(HTTP_1_1, FORBIDDEN, forbiddenByteBuf.retainedDuplicate());
            } else {
                res = new DefaultFullHttpResponse(HTTP_1_1, FORBIDDEN);
        ...
        //path match
        String pattern = null;
        Set<WsPathMatcher> pathMatcherSet = wsEndpointDispatcher.getPathMatcherSet();
        ...
        if (wsEndpointDispatcher.hasBeforeHandshake(channel, pattern)) {
            wsEndpointDispatcher.doBeforeHandshake(channel, req, pattern);
            if (!channel.isActive()) {
                return;
            }
        ...
        if (handshaker == null) {
            ...
            if (config.isUseEventExecutorGroup()) {
                pipeline.addLast(eventExecutorGroup, new WebSocketServerHandler(wsEndpointDispatcher));
            } else {
                pipeline.addLast(new WebSocketServerHandler(wsEndpointDispatcher));
            }
            ...
            handshaker.handshake(channel, req).addListener(future -> {
                if (future.isSuccess()) {
                    ...
                    wsEndpointDispatcher.doOnOpen(channel, req, finalPattern);
                } else {
