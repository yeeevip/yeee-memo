package vip.yeee.memo.integrate.common.mep.interceptor;
...
import vip.yeee.memo.integrate.common.mep.utils.JsonUtils;
import vip.yeee.memo.integrate.common.mep.utils.PluginUtils;
...
public class SensitiveAndEncryptWriteInterceptor implements Interceptor {
    ...
    private void handleParameters(Configuration configuration, BoundSql boundSql,Object param,SqlCommandType commandType) throws Exception {
        ...
        for (Field field : fields) {
            ...
            if (value instanceof CharSequence) {
                ...
                if (isWriteCommand(commandType) && !SensitiveTypeRegistry.alreadyBeSensitive(newValue)) {
    ...
    private Object handleSensitiveField(Field field, Object value) {
        ...
        if (sensitiveField != null && value != null) {
            newValue = SensitiveTypeRegistry.get(sensitiveField.value()).handle(value);
        }
        return newValue;
    }
    ...
    private Object processJsonField(Object newValue,SensitiveJSONField sensitiveJSONField) {
        ...
        try{
            ...
            SensitiveJSONFieldKey[] keys = sensitiveJSONField.fieldKeyList();
            for (SensitiveJSONFieldKey jsonFieldKey :keys) {
                ...
                if (oldData != null) {
                    String newData = SensitiveTypeRegistry.get(sensitiveType).handle(oldData);
                    map.put(key,newData);
                }
            }
